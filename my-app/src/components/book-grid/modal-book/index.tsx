...
const ModalBook: React.FC<Props> = ({ book, cover, open, onClose }) => {
    ...
    React.useEffect(() => {
        if (open) {
            getShareLink(book.id).then(setUrl);
        }
    }, [open, book.id]);
        ...
        >
            {url && (
                <div className="flex gap-8 p-4 items-center">
                    {/* Левая секция - Обложка книги */}
                    <div className="flex flex-col items-center gap-3">
                        <Button
                            type="link"
                            className="text-blue-600 underline text-sm p-0 h-auto"
                            onClick={() => {
                                onClose();
                                navigate(`/books/${book.id}`, {
                                    state: {
                                        title: `${book.author.toUpperCase()} ${book.title.toUpperCase()}`,
                                        pdfUrl: book.pdfUrl || ''
                                    }
                                });
                            }}
                        ...
                        <p className="text-sm text-gray-700 text-center max-w-32">
                            {book.author}. {book.title}
                        </p>
                    </div>

                    {/* Центральная секция - QR код */}
                    ...
                    <div className="flex flex-col gap-2 text-sm items-center justify-center">
                        <p className="font-semibold  text-gray-800 text-center">#QyzyljarCreativeCenter</p>
                        <div className="flex items-center gap-2 justify-center">
                            <InstagramIcon />
                            <span >@qyzyljar.creativecenter</span>
                        </div>
                        <div className="flex items-center gap-2 justify-center">
                            <TikTokIcon />
                            <span>@qyzyljar.creativecenter</span>
